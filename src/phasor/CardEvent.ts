...
// Mutates the command stack by pushing state changes.
    ...
    deck.cards.forEach((card) => {
      card.on(
        ...
        (_pointer: Phaser.Input.Pointer, _dragX: number, _dragY: number) =>
          executeIfMovable(dragStart)(card),
      );
      card.on(
        "dragend",
        (_pointer: Phaser.Input.Pointer, _dragX: number, _dragY: number) =>
          executeIfMovable(dragEnd)(card),
      );
      card.on(
        "drag",
        (_pointer: Phaser.Input.Pointer, dragX: number, dragY: number) =>
          executeIfMovable(drag)(card, dragX, dragY),
      );
      card.on(
        "drop",
        (
        ...
        ) => executeIfMovable(drop)(card, dropZone),
      );
      card.on("pointerdown", (_pointer: Phaser.Input.Pointer) => {
        executeIfMovable(snap)(card);
      });
    });
  }
  
  // Everything below this point is a pure function.
  function dragStart(card: Card, deck: Deck): Command | null {
    deck.cardChildren(card).forEach((child, i) => child.setDepth(100 + i));
    return null;
  }
  
  function dragEnd(card: Card, deck: Deck): Command | null {
